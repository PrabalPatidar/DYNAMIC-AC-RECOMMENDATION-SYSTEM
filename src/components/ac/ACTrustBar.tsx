...
const trustItems = [
  ...
  },
  {
    ...
    title: 'Built for Ahmedabad',
    desc: 'Handles heat up to 48°C',
  },
  {
    icon: (
    ...
    desc: 'Genuine products & warranty',
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-purple-600">
...
export default function ACTrustBar() {
  return (
    <motion.div
      className="ac-trust-bar"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, delay: 0.2 }}
    >
      {trustItems.map((item, i) => (
        <div key={i} className="ac-trust-item">
          <div className="ac-trust-icon">{item.icon}</div>
          <div>
            <div className="ac-trust-title">{item.title}</div>
            <div className="ac-trust-desc">{item.desc}</div>
          </div>
        </div>
      ))}
    </motion.div>
